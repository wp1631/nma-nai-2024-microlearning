...
from models import calculate_accuracy
...
if __name__ == "__main__":
    ...
    (losses_perturb[:], accuracy_perturb[:], _, snr_perturb) = node_perturb_mlp.train(
        rng_wp,
        train_images,
        train_labels,
        numepochs,
    ...
    with plt.xkcd():
        ...
        fig.suptitle("Test plot KP loss", fontsize=16)
        plt.show()

    print("Loss:", losses_perturb[-1])
    print("Acc:", accuracy_perturb[-1])
    test_loss = node_perturb_mlp.mse_loss_batch(rng_wp, test_images, test_labels)
    test_loss_sd1 = node_perturb_mlp.mse_loss_batch(
        rng_wp, test_images + np.random.normal(0, 1, test_images.shape), test_labels
    )
    test_loss_sd2 = node_perturb_mlp.mse_loss_batch(
        rng_wp, test_images + np.random.normal(0, 2, test_images.shape), test_labels
    )
    _, test_result = node_perturb_mlp.inference(rng_wp, test_images)
    _, test_result_sd1 = node_perturb_mlp.inference(
        rng_wp, test_images + np.random.normal(0, 1, test_images.shape)
    )
    _, test_result_sd2 = node_perturb_mlp.inference(
        rng_wp, test_images + np.random.normal(0, 2, test_images.shape)
    )
    print("Test Loss:", np.mean(test_loss))
    print("Test Loss Sd1:", np.mean(test_loss_sd1))
    print("Test Loss Sd2:", np.mean(test_loss_sd2))
    print("Test Acc:", calculate_accuracy(test_result, test_labels))
    print("Test Acc Sd1:", calculate_accuracy(test_result_sd1, test_labels))
    print("Test Acc Sd2:", calculate_accuracy(test_result_sd2, test_labels))
    pass
